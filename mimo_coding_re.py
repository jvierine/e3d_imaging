...
def convolution_matrix(envelope, rmax=300):
    ...
    idx=n.arange(L)
    for i in n.arange(L):
        gidx=idx[n.where( (i-ridx >= 0) )[0]]
        A[i, gidx] = envelope[ i-ridx[gidx] ] 
...
def sim_isr(N_pulses=5000,N_tx=2,N_r=200,L=32):
    """ 
    ...
    for i in range(N_pulses):
        ...
        m0=n.dot(A0,z0)+n.dot(A1,z1)

        tx00=n.zeros(len(m0),dtype=n.complex64)
        tx11=n.zeros(len(m0),dtype=n.complex64)
        tx01=n.zeros(len(m0),dtype=n.complex64)
        tx10=n.zeros(len(m0),dtype=n.complex64)        
        ...
        if False:
            plt.subplot(121)
            plt.plot(n.real(tx01) - n.imag(tx10))
            plt.subplot(122)
            plt.plot(n.imag(tx01) + n.real(tx10))
            plt.show()
        ...
        ml=n.zeros(len(m0),dtype=n.complex64)

        ml[0:(len(m0)-1)] = m0[0:(len(m0)-1)]*n.conj(m0[1:len(m0)])
...
#        plt.plot(m_im[(N_r*i):(N_r*i+N_r)])
 #       plt.show()
    ...
    plt.plot(n.sqrt(var_re*n.diag(Sinv)))
    plt.show()
...
#    xhat=n.linalg.lstsq(A,m.flatten())[0]
    ...
    plt.plot(z00.real/N_pulses,label="true (re)")
    plt.plot(z00.imag/N_pulses,label="true (im)")
    plt.legend()
    ...
    plt.plot(z11.real/N_pulses,label="true (re)")
    plt.plot(z11.imag/N_pulses,label="true (im)")
    plt.legend()
    ...
    plt.plot(z01.real/N_pulses,label="true (re)")
    plt.plot(z01.imag/N_pulses,label="true (im)")
    plt.legend()
